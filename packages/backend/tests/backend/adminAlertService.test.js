...
describe("AdminAlertService", () => {
  ...
  describe("Manual failure management", () => {
    ...
    it("should get all active failures", () => {
      ...
      expect(activeFailures).toBeInstanceOf(Map);
      expect(activeFailures.size).toBe(2);
      expect(activeFailures.get(fpmm1)).toBe(2);
      expect(activeFailures.get(fpmm2)).toBe(1);
    });
  });
  ...
  describe("Error handling", () => {
    ...
    it("should track error details in failure record", () => {
      ...
      expect(failures.get(fpmmAddress)).toBe(1);
    });
  });
